...
class M_Product extends CI_Model {
    ...
    function get($search_token = FALSE, $filter = array(), $limit = 999, $offset = 0) {
        $this->db->select('product.status, product.id, product.description, product.code, f.formulation_code, product.reorder_level, fk_production_formulation_id, product.formulation_code AS formulation, cost_method');
   ...
   public function category_in($category_ids)
   {
    ...
    public function get_logs($product_id, $page = 1)
    {
        ...
        $this->db->join('production_receiving_details AS prr', 'prr.id = production_receiving_detail_id', 'left');
        $this->db->join('production_job_order_detail AS jo', 'jo.id = inv.jo_order_detail_id', 'left');
        $this->db->join('inventory_stock_withdrawal_detail AS w', 'w.id = inv.withdrawal_detail_id', 'left');
    ...
    {
        $this->db->select('p.id, p.description, p.code, u.description AS unit_description, f.formulation_code')->from('inventory_product AS p');
        $this->db->join('inventory_unit AS u', 'u.id = p.fk_unit_id');
        $this->db->join('production_formulation AS f', 'f.id = p.fk_production_formulation_id', 'left');
