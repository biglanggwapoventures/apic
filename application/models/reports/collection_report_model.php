...
class Collection_report_model extends CI_Model
{
	...
	{
		$this->db->select('receipt.id, s_order.id AS so_id, receipt.deposit_date, DATE(delivery.`date`) AS delivery_date, receipt_detail.fk_sales_delivery_id, receipt_detail.amount, customer.company_name AS customer_name, DATEDIFF(receipt.deposit_date, DATE(delivery.`date`)) AS days_collected, agent.name AS sales_agent', FALSE)
			...
			->order_by('receipt.deposit_date ASC, customer.company_name ASC, receipt_detail.amount DESC');
