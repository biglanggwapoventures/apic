...
class Collection_report_model extends CI_Model
{
	...
	{
		$this->db->select('receipt.id, receipt.deposit_date, DATE(delivery.`date`) AS delivery_date, receipt_detail.fk_sales_delivery_id, receipt_detail.amount, customer.company_name AS customer_name, DATEDIFF(receipt.deposit_date, DATE(delivery.`date`)) AS days_collected', FALSE)
