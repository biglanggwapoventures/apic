...
class medications extends PM_Controller_v2 {
    ...
    protected $view_url = 'maintainable/';

    function __construct() {
        parent::__construct();
        $this->load->model('maintainable/Medication_model', 'medication');
    }

    public function index() {
        $this->setTabTitle('Medications');
        ...
        $this->add_javascript([
            ...
            'maintainable/medications/medications.js',
        ]);
        $this->set_content($this->view('medications'));
        $this->generate_page();
    }
    ...
    public function ajax_initialize() {
        ...
        $data['medications'] = $this->medication->all();
        $this->output->set_content_type('json')->set_output(json_encode($this->response(FALSE, '', $data)));
    }
