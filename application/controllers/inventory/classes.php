...
class Classes extends PM_Controller {
    ...
    private $_subject;
    private $protected_classes = [1, 2, 3, 4];
    ...
    public function __construct() {
        ...
        $this->_subtitle = 'Classes';
        $this->_subject = 'product class';
        $this->_additional_css = array('bootstrap-editable.css');
        $this->_additional_js = array(
            'inventory-maintainables.js',
            'jquery.form.min.js',
            'bootstrap-editable.min.js');
        $this->load->model('inventory/m_class');
    }

    function index() {
        $data['default_keyword'] = $this->input->get('search_keyword');
        $data['entries'] = $this->m_class->get($data['default_keyword']);
        $this->set_data($this->_nav, $this->_subtitle, $this->_additional_css, $this->_additional_js);
        $this->load_page($this->load->view('inventory/classes', $data, TRUE));
    }

    function a_add() {
        $this->form_validation->set_rules('description', 'Description', 'required|is_unique[inventory_class.description]');
        if ($this->form_validation->run()) {
            $return_id = $this->m_class->add($this->input->post('description'));
            if ($return_id) {
    ...
    function a_update() {
        $this->form_validation->set_rules('value', 'Description', 'required|is_unique[inventory_class.description]');
        $this->form_validation->set_rules('pk', 'ID', 'required');
        if ($this->form_validation->run()) {
            $id = $this->input->post('pk');
            if (in_array($id, $this->protected_classes)) {
                echo json_encode($this->response(TRUE, $this->m_message->update_exception($this->_subject)));
                return;
            }
            $updated = $this->m_class->update($id, $this->input->post('value'));
            if ($updated) {
    ...
    function a_delete() {
        ...
        if ($this->form_validation->run()) {
            $id = $this->input->post('pk');
            if (in_array($id, $this->protected_classes)) {
                echo json_encode($this->response(TRUE, $this->m_message->delete_exception($this->_subject)));
                exit();
            }
            $deleted = $this->m_class->delete($id);
            if ($deleted) {
