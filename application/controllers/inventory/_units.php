...
class Units extends PM_Controller {
    ...
    public function __construct() {
        ...
        $this->_subtitle = 'Units';
        $this->_subject = 'product unit';
    ...
    function index() {
        ...
        $data['entries'] = $this->m_unit->get($data['default_keyword']);
        $this->set_data($this->_nav, $this->_subtitle, $this->_additional_css, $this->_additional_js);
        $this->load_page($this->load->view('inventory/units', $data, TRUE));
    }

    function a_add() {
        $this->form_validation->set_rules('description', 'Description', 'required|is_unique[inventory_unit.description]');
        if ($this->form_validation->run()) {
            $return_id = $this->m_unit->add($this->input->post('description'));
            if ($return_id) {
    ...
    function a_update() {
        $this->form_validation->set_rules('value', 'Description', 'required|is_unique[inventory_unit.description]');
        $this->form_validation->set_rules('pk', 'ID', 'required');
        if ($this->form_validation->run()) {
            $updated = $this->m_unit->update($this->input->post('pk'), $this->input->post('value'));
            if ($updated) {
    ...
    function a_delete() {
        $this->form_validation->set_rules('pk', 'ID', 'required');
        if ($this->form_validation->run()) {
            $deleted = $this->m_unit->delete($this->input->post('pk'));
            if ($deleted) {
