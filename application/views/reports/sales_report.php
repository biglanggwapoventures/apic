...
<input type='hidden' name='data-url-for-printing' value='<?= base_url('reports/sales_report/generate') ?>'>
<div class="row">
    <div class="col-md-12">
                    ...
                    <tbody data-action-url='<?= base_url('production/job_orders/a_do_order') ?>'>
                        <tr><th>Date</th><th>DR#</th><th>Customer</th><th>Check Number</th><th>Bank/Branch</th><th>Check Date</th><th>Amount</th></tr>
                            <?php 
                                ...
                                if($sales_delivery_list){
                            ?>
                            <?php foreach ($sales_delivery_list as $item): ?>
                                <tr data-pk='<?= $item['id'] ?>'>
                                    <td><?= $item['date'] ?></td>
                                    <td><?= $item['id'] ?></td>
