...
<div class="box box-solid"
     ...
     data-get-customer-unsettled-url="<?= base_url('trucking/payments/ajax_get_unsettled') ?>">
     ...
     <form data-action="<?= $form_action ?>">
        <div class="box-body">
            <div class="callout callout-danger hidden"><ul class="list-unstyled"></ul></div>
             <div class="row">
                <div class="col-md-8">
                    <div class="row">
                        <div class="col-sm-6">
                            <?php if( isset($payments_prev_info) ) { ?>
                            <?= ($payments_prev_info) ? '<a href="'.$payments_prev_info.'"><i class="fa fa-arrow-left"></i> Go to SR # '.$payments_prev_id.'</a>' : '' ?>
                            <?php } ?>
                        </div>
                        <div class="col-sm-6 text-right">
                            <?php if(isset($payments_next_info)) { ?>
                            <?= ($payments_next_info) ? '<a href="'.$payments_next_info.'">Go to SR # '.$payments_next_id.' <i class="fa fa-arrow-right"></i></a>' : '' ?>
                            <?php } ?>
                        </div>
                    </div>
                    <?= ( (isset($payments_prev_info) || isset($payments_next_info)) && (!empty($payments_prev_info) || !empty($payments_next_info)) ) ? "<hr/>" : "" ?>
                    <div class="row">
                        <div class="col-sm-3">
                            <div class="form-group">
                                <label for="date">Date</label>
                                <?php $date = isset($data['payment']['date']) ? date('m/d/Y', strtotime($data['payment']['date'])) : date('m/d/Y')?>
                                <input type="text" class="form-control datepicker" value="<?= $date ?>" required="required" name="date"/>
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <?php if(isset($data['payment']['customer'])):?>
                                    <p class="form-control-static"><?= $data['payment']['customer']?></p>
                                <?php else:?>
                                    <?= trucking_customer_dropdown('customer', FALSE, 'class="form-control" id="search-customer" required="required"')?>
                                <?php endif;?>
                            </div>
                        </div>
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <?php $tracking_type = isset($data['payment']['tracking_type']) ? $data['payment']['tracking_type'] : FALSE;?>
                        ...
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <?php $tracking_no = isset($data['payment']['tracking_no']) ? $data['payment']['tracking_no'] : FALSE;?>
                     ...
                     <div class="form-group">
                        <label for="remarks">Remarks</label>
                         <?php $remarks = isset($data['payment']['remarks']) ? $data['payment']['remarks'] : FALSE;?>
                    ...
                    <div class="box box-solid box-primary">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                ...
                                <?php $payment_type = !empty($data['payment']['check']) ? 'check' : 'cash'; ?>
                            ...
                            <div class="form-group">
                                ...
                                <?php $account = isset($data['payment']['pay_from']) ? $data['payment']['pay_from'] : ''; ?>
                                <input type="text" value="<?=$account?>" class="form-control"  name="pay_from"/>
                            </div>
                            <div class="form-group">
                                <label>Check Amount</label>
                                <?php $amount = isset($data['payment']['check']['check_amount']) ? $data['payment']['check']['check_amount'] : 0; ?>
                            ...
                            <div class="form-group">
                                ...
                                <?php $check_num = isset($data['payment']['check']['check_number']) ? $data['payment']['check']['check_number'] : ''; ?>
                            ...
                            <div class="form-group">
                                ...
                                <?php $check_date = isset($data['payment']['check']['check_date']) ? date('m/d/Y', strtotime($data['payment']['check']['check_date'])) : ''; ?>
                             ...
                             <div class="form-group">
                                ...
                                <?php $pay_to = isset($data['payment']['pay_to']) ? $data['payment']['pay_to'] : FALSE; ?>
                                <?= arr_group_dropdown('pay_to', $bank_accounts, 'id', 'bank_name', $pay_to, FALSE, 'class="form-control" required="required"') ?>
                            </div>
                            <div class="form-group">
                                <label>Deposit Date</label>
                                <?php $deposit_date = isset($data['payment']['deposit_date']) && $data['payment']['deposit_date'] ? date('m/d/Y', strtotime($data['payment']['deposit_date'])) : ''; ?>
            ...
            <?php if ($this->session->userdata('type_id') == M_Account::TYPE_ADMIN): ?>
                <?php $checked = isset($data['payment']['status']) && $data['payment']['status'] == M_Status::STATUS_FINALIZED ? 'checked="checked"': ''?>
                <div class="checkbox">
                    <label>
                        <input name="status" value="<?= M_Status::STATUS_FINALIZED ?>" type="checkbox" <?=$checked?>/> Mark this sales payment as finalized
                    </label>
                </div>
            <?php endif; ?>
        </div>
        <div class="box-footer clearfix">
            ...
            <a href="<?= base_url('trucking/payments') ?>" class="btn btn-danger pull-right btn-flat cancel">Go back</a>
        </div>
    </form>
</div>