<?php $pl_link = base_url('trucking/packing_list/get');?>
<?php if (!empty($data)): ?>
    <?php $total_balance = 0; ?>
                ...
                <?php foreach ($row['payments'] as $index => $payment): ?>
                    ...
                    <td class="text-center<?=$td_class?>">
                        <?php if(isset($payment['id'])):?>
                            <a target="_blank" href="<?=base_url("trucking/payments/update/{$payment['id']}")?>"><?= "{$payment['tracking_type']} {$payment['tracking_number']}"?></a>
                        <?php else:?>
