        ...
        fetchData = function(){
            ...
            request.done(function(response){
                ...
                $.each(response, function(i, v){
                    ...
                    td[5] = v.created_by;
                    td[6] = v.approved_by;
                    td[7] = v.approved_by !== null ? '<span class="label label-success">Approved</span>' : '<span class="label label-warning">Pending</span>';
                    td[8] = (isAdmin ? deleteTemplate : ''); //+ (actionTemplate(v.approved_by !== null, v.id));
                    tr.push('<tr data-pk="'+v.id+'"><td>'+td.join('</td><td>')+'</td></tr>');
                })
                if(response.length){
                    table.find('tbody').html(tr.join(''));
                }else{
                    table.find('tbody').html('<tr><td colspan="9" class="text-center">No more data to display</td></tr>');
                }
                $('.print').printPage();
            });
