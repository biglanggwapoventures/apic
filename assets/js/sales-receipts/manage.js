    ...
    var getUnsettled_URL = $('.box').data('get-customer-unsettled-url'),
        ...
        populateTransactions = function(data){
            ...
            $.each(data, function(i, v){
                var td = [];
                td[0] = '<input type="hidden" value="'+v.fk_sales_delivery_id+'" name="details['+v.fk_sales_delivery_id+'][pl_id]"/><a target="_blank" href="'+gotoPL(v.fk_sales_delivery_id)+'">PL# '+v.fk_sales_delivery_id+'</a>';
                td[1] = v.date;
                ...
                td[5] = '<input type="text" class="form-control price payment" name="details['+v.fk_sales_delivery_id+'][this_payment]"/>';
